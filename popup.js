...
document.addEventListener('DOMContentLoaded', function() {
  ...
  async function readSheetData(spreadsheetId, sheetName) {
    try {
      if (window.authenticateWithGoogle) {
  ...
  chrome.storage.sync.get(['spreadsheetId', 'sheetName'], function(result) {
    if (result.spreadsheetId) {
      ...
      if (spreadsheetIdField) {
        spreadsheetIdField.value = result.spreadsheetId;
      }
    }
    if (result.sheetName) {
      const sheetNameField = document.getElementById('sheet-name');
      if (sheetNameField) {
        sheetNameField.value = result.sheetName;
      }
    }
  });
  ...
  chrome.storage.sync.get(['aiApiKey'], function(result) {
    if (result.aiApiKey) {
      ...
      if (aiApiKeyField) {
        aiApiKeyField.value = result.aiApiKey;
      }
    }
  });

  // Name format radio button event listeners
  ...
  // Save AI key button (alternative ID)
    ...
    if (window.authenticateWithGoogle) {
      window.authenticateWithGoogle(async function(token) {
        try {
          ...
          const sheetAnalysisDiv = document.getElementById('sheet-analysis');
          if (sheetAnalysisDiv) {
            sheetAnalysisDiv.innerHTML = `
          ...
          if (headerRowSelector) {
            headerRowSelector.value = bestHeaderRow;
          }
...
function populateColumnSelectors(headers, analysis) {
  ...
  if (analysis.hasFirstNameColumn && analysis.hasLastNameColumn) {
    const nameSeparateRadio = document.getElementById('name-separate');
    if (nameSeparateRadio) {
      nameSeparateRadio.checked = true;
      showNameFormatConfig('separate');
    }
  } else if (analysis.hasFullNameColumn) {
    const nameCombinedRadio = document.getElementById('name-combined');
    if (nameCombinedRadio) {
      nameCombinedRadio.checked = true;
...
function showNameFormatConfig(format) {
  ...
  if (separateConfig && combinedConfig) {
    if (format === 'separate') {
...
function saveRSVPConfiguration() {
  ...
  const config = {
    ...
    headerRowIndex: parseInt(headerRowSelector?.value || '0'),
    sheetName: window.currentSheetName || 'Sheet1' // Save the sheet name that was analyzed
  };
  
  if (nameFormat === 'separate') {
    const firstNameColumn = document.getElementById('first-name-column');
    const lastNameColumn = document.getElementById('last-name-column');
    
    config.firstNameColumn = firstNameColumn?.value;
    config.lastNameColumn = lastNameColumn?.value;
    ...
    const fullNameColumn = document.getElementById('full-name-column');
    config.fullNameColumn = fullNameColumn?.value;
  ...
  const rsvpResponseColumn = document.getElementById('rsvp-response-column');
  config.rsvpResponseColumn = rsvpResponseColumn?.value;
  ...
  const yesMapping = document.getElementById('yes-mapping');
  const noMapping = document.getElementById('no-mapping');
  const unsureMapping = document.getElementById('unsure-mapping');
  
  config.rsvpMapping = {
    yes: yesMapping?.value || '',
    no: noMapping?.value || '',
    unsure: unsureMapping?.value || ''
  };
      ...
      if (window.authenticateWithGoogle) {
        window.authenticateWithGoogle(async function(token) {
          try {
            ...
            if (window.testWritePermissions) {
            ...
            let nextEmptyRow;
            if (window.findNextEmptyRow) {
              nextEmptyRow = await window.findNextEmptyRow(token, result.spreadsheetId, sheetName, headerRowIndex + 1);
            } else {
              nextEmptyRow = headerRowIndex + 2; // Fallback
            }
...
function fetchMessages() {
  chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
    ...
    if (!tabs[0]) {
      ...
      if (messagesDiv) {
        messagesDiv.textContent = 'No active tab found.';
      }
      return;
    }
...
function showRSVPConfirmation(names, rsvpAnalysis, originalMessages) {
  ...
  let displayHTML = '<h4>Extracted Names:</h4>' +
    '<p>' + names.join(', ') + '</p>' +
  ...
  names.forEach((name, index) => {
    displayHTML += '<div style="margin: 8px 0; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">' +
...
async function updateGuestRSVP(index) {
  try {
    const nameField = document.getElementById('guest-name-' + index);
    const rsvpField = document.getElementById('guest-rsvp-' + index);
    
    if (!nameField || !rsvpField) {
      alert('Error: Could not find form fields.');
      return;
    }
    
    const name = nameField.value.trim();
    const rsvpStatus = rsvpField.value;
      ...
      if (window.authenticateWithGoogle) {
        window.authenticateWithGoogle(async function(token) {
          try {
            ...
            let dropdownOptions = [];
            if (window.getColumnDropdownOptions) {
              dropdownOptions = await window.getColumnDropdownOptions(token, result.spreadsheetId, sheetName, columnLetter);
            }
            
            if (dropdownOptions.length > 0) {
              populateDropdownMapping(dropdownOptions);
              const rsvpMappingConfig = document.getElementById('rsvp-mapping-config');
              if (rsvpMappingConfig) {
                rsvpMappingConfig.style.display = 'block';
              }
            } else {
...
function resetSetup() {
  if (confirm('Are you sure you want to reset the setup? This will clear all your configuration and return you to the onboarding process.')) {
    ...
    chrome.storage.sync.remove(['rsvpConfig', 'spreadsheetId'], function() {
      ...
      const setupSpreadsheetId = document.getElementById('setup-spreadsheet-id');
      if (setupSpreadsheetId) {
        setupSpreadsheetId.value = '';
      }
      
      const headerRowSelector = document.getElementById('header-row-selector');
      if (headerRowSelector) {
        headerRowSelector.innerHTML = '<option value="">Select header row...</option>';
      }
      
      const rsvpMappingConfig = document.getElementById('rsvp-mapping-config');
      if (rsvpMappingConfig) {
        rsvpMappingConfig.style.display = 'none';
      }
...
async function testCalendarAccess() {
  try {
    if (window.authenticateWithGoogle) {
      window.authenticateWithGoogle(async function(token) {
        try {
          let calendars, primaryCalendar;
          if (window.listCalendars) {
            calendars = await window.listCalendars(token);
          }
          if (window.getPrimaryCalendar) {
            primaryCalendar = await window.getPrimaryCalendar(token);
          }
          
          const resultsDiv = document.getElementById('calendar-docs-results');
          if (resultsDiv) {
            resultsDiv.innerHTML = `
              <div class="setup-status success">
                ✅ Calendar access confirmed!
                <br><strong>Primary Calendar:</strong> ${primaryCalendar ? primaryCalendar.summary : 'Unknown'}
                <br><strong>Available Calendars:</strong> ${calendars ? calendars.items.length : 'Unknown'}
      ...
      window.authenticateWithGoogle(async function(token) {
        try {
          ...
          if (window.createDocument) {
            doc = await window.createDocument(token, 'Test Document');
          }
          ...
          if (resultsDiv) {
            resultsDiv.innerHTML = `
              <div class="setup-status success">
                ...
                <br><strong>Test Document Created:</strong> ${doc ? doc.title : 'Unknown'}
                <br><strong>Document ID:</strong> ${doc ? doc.documentId : 'Unknown'}
    ...
    if (window.authenticateWithGoogle) {
      window.authenticateWithGoogle(async function(token) {
        try {
          ...
          let event;
          if (window.createWeddingEvent) {
            event = await window.createWeddingEvent(token, 'primary', eventDetails);
          }
          ...
          if (resultsDiv) {
            resultsDiv.innerHTML = `
              <div class="setup-status success">
                ...
                <br><strong>Event:</strong> ${event ? event.summary : 'Unknown'}
                <br><strong>Start:</strong> ${event ? event.start.dateTime : 'Unknown'}
                <br><strong>Event ID:</strong> ${event ? event.id : 'Unknown'}
  ...
  try {
    if (window.authenticateWithGoogle) {
      window.authenticateWithGoogle(async function(token) {
        try {
          let doc;
          if (window.createWeddingPlanningDoc) {
            doc = await window.createWeddingPlanningDoc(token, 'Wedding Planning Document');
          }
          ...
          if (resultsDiv) {
            resultsDiv.innerHTML = `
              <div class="setup-status success">
                ...
                <br><strong>Document:</strong> ${doc ? doc.title : 'Unknown'}
                <br><strong>Document ID:</strong> ${doc ? doc.documentId : 'Unknown'}
                <br><strong>URL:</strong> <a href="https://docs.google.com/document/d/${doc ? doc.documentId : ''}" target="_blank">Open Document</a>
